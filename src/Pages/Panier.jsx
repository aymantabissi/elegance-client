...
function Panier() {
  ...
  const getImageUrl = (image) => {
    if (!image) return `${API_URL}/uploads/default-image.jpg`;

    if (image.startsWith("http")) {
      return image.replace("http://localhost:5000", API_URL);
    }
        ...
        <div className="max-w-5xl mx-auto">
          <div className="bg-white p-6 shadow-lg rounded-lg overflow-x-auto">
                  ...
                  <tr key={item._id} className="border-t">
                    <td className="p-4 flex items-center">
                      <img
                        ...
                        alt={item.name}
                        className="w-16 h-16 rounded-md shadow-md mr-4 object-cover"
                        onError={(e) => {
                    ...
                    <td className="p-4">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          updateQuantity(
                            item._id,
                            Math.max(1, Math.min(10, newQuantity))
                          );
                        }}
                      />
                    </td>
            ...
            <div className="flex justify-between mt-6">
              <Link
                to="/"
                className="border px-4 py-2 rounded-md shadow-md hover:bg-gray-200"
              >
                Return To Shop
              </Link>

              <button className="border px-4 py-2 bg-blue-500 text-white rounded-md shadow-md hover:bg-blue-600">
                Update Cart
              </button>
            </div>
          </div>
          ...
          <div className="mt-6 flex gap-6 flex-col md:flex-row">
            <div className="flex-1">
              ...
              />
              <button className="mt-2 w-full bg-red-500 text-white py-2 rounded-lg shadow-md hover:bg-red-600 transition">
                Apply Coupon
              </button>
            </div>
